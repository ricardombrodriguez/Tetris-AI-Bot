...
class Search:
    ...
    def search(self, shape=None, sol=None):
        ...
        #new_solution = 0
        #print("Begin shape")
        #print(shape) if shape != None else print(self.shape)
        ...
        for rot in range(0, len(self.shape.plan)):
            ...
            #print("current total heuristic:", sol) if sol != None else print("current herustic: 0")
            ...
            for x in range(1, self.x-1):
                ...
                elif self.biggestHeight(solution) < 6:
                    solution.heuristic = (self.checkHeight(solution) * -0.210066) + (self.checkBumpiness(solution) * -0.184483) + (self.checkHoles(solution)* -0.65663) + (self.checkScore(solution) * 0.555)
                else:
            ...
            # if shape == None:        
            #     print("Found best solution for ", solution.shape,  "heuristic", solution.heuristic)
            # elif sol != None:
            #     print("Found best solution2 for ", shape,  "heuristic", sol)
            
            # print("End shape")
            # new_solution = copy(solution.heuristic)
            # while self.next_shapes != []:
            #     print("Searching next shape with current heuristic:", new_solution)
    ...
    def checkBumpiness(self,solution):
        bumpiness = 0
        #this_height = self.y-1
        #next_height = self.y-1
